...
Now, switch back to the **Extension Development Host** window and invoke the "Hello World" command again. Once execution reaches the breakpoint, VS Code will stop and focus on  The new VS Code window will open shortly, the tests will be run and then the window will be closed again. You can see the results of the tests in the **Debug console**.the target line in the editor. Here we can inspect the state and step through the code in the built-in debugger. For examplg Console** REPL](https://code.visualstudio.com/docs/editor/debugging#_debug-console-repl) and have them evaluated in  we have previously used `F5` the current execution context. For example, type `words` and then expand the evaluated `Array` value to see th to launch  elements in the array.
...
- Learn how to use logpoints.
  ...
  - Put a logpoint in `src/extension.ts` on the line that shows the notification. Make it log some The new VS Code window will open shortly, the tests will be run and then the window will be closed again. You can see the results of the tests in the **Debug console**.thing like `Words: ` followed by the actual words in the `words` variable (without hard-coding them in the logpoint)e the "Hello World" command and inspect the logpoint output (in the **Debug console**). we have previously used `F5` to launch
...
3. Provide a `Definition`, which is basically a [Location](https://code.visualstudio.com/api/references/vscode-api#Location) object pointing to the URI of the template file and a `Range` or `Position`. (Here we use position (0, 0) because the whole file represents the definition of the template URL).
...
> **NOTE:**<br />
> You may remember that we have previously used `F5` to run the extension (in a new **Extension Development Host** window). In fact, `F5` starts the debug target that is currently selected in **Debug view**'s dropdown. The dropdown is populated from the configurations found in [.vscode/launch.json](../.vscode/launch.json).

Now that we have seen how to run the tests, let's better understand the files involved:
- [runTest.ts](../src/test/runTest.ts) calls the `runTests()` helper from [vscode-test][vscode-test] (a package that provides helpers for testing VS Code extensions). This will launch the **Extension Development Host** window and pass necessary options.
...
The scaffolded extension comes with [Mocha][mocha] as the test runner, so we will go with that.
...
>
> For Jasmine (whose default reporter uses `process.stdout.write()`), this means that extra work is needed to have test results be reported correctly. Discussing this in more detail is beyond the scope of this introductory workshop, but you could see Jasmine being used as the test runner for an extension [here](https://github.com/gkalpak/aio-docs-utils/blob/bb638edbf/src/test/helpers/jasmine-runner.ts).
...
- Open `fixtures/simple-component/` in test VS Code instances.
...
Let's now write some tests for the `extension.helloWorld` command and the provided hovers. We will also install [Sinon.JS](https://sinonjs.org/) to be able to use spies and stubs in our tests. Run the following command to install Sinon.JS its typings (for intellisense):
...
As we have seen in [Lab 3](#lab-3), we can put a breakpoint inside the `provideHover()` method, run the e2e tests again. By stepping through the code in the debugger, we find out that the target line does not contain the `templateUrl` property, but `styleUrls`. It now makes sense that the extension did not provide any hover.

We now realize that the position passed to the `vscode.executeHoverProvider` command in `e2e/extension.test.ts` is incorrect. This is because lines in a document are 0-indexed, so in order to target the 5th line we should pass `4` (not `5`).

You can now remove the breakpoint, fix the test code and rerun the tests. Everything should pass this time.

Phew! At least the bug was in the test and not in the extension code 😅
...
- Add a test to verify that the `DefinitionProvider` works.
  ...
  - Add assertions to verify that the returned definition targets the whole template HTML file.
...
Without getting into details, `mock-require` allows you to specify an arbitrary value to be returned when a package is imported. We will modify [src/test/unit/index.ts](../src/test/unit/index.ts) to provide a mock value (an empty object) importing `vscode`.
...
##
<sub>&emsp;_**7.c: Add test for `extension.ts`.**_ | Git tag: `lab-7.c`</sub>
...
> ✔️ **Best practice**<br />
> Ideally, you want unit tests to be run on every change, so that you get immediate feedback. Manually running the command every time is tedious. Therefore, it is a good idea to use a package such as [watch](https://www.npmjs.com/package/watch) to have an npm script that watches the `out/` directory and reruns the unit tests on every change; e.g.: `"test-unit-watch": "watch \"npm run test-unit\" out/ --wait 1"`

##
<sub>&emsp;_**7: Add more tests for `extension.ts`.**_ | Git tag: `lab-7`</sub>

Let's add some more tests for `extension.ts`. The main pattern is that we stub the `vscode` API we need to run assertions against, then we call `activate()` or any other function we need and finally we verify that the right API were called.
